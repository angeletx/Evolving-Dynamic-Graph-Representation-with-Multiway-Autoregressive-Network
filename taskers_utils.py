...
def update_edges_mask(prev_edges_mask, adj_list):
    num_nodes = adj_list[0].shape[0]
    for i,cur_adj in enumerate(adj_list):
        index = cur_adj._indices().detach().cpu()
        current_edge_ids = get_edges_ids(index, num_nodes)
        prev_edges_mask[current_edge_ids] = True 

    return prev_edges_mask 
    
def get_statistics(ground_truth, prev_mask):
    print('prev mask: {}'.format(np.sum(prev_mask)))
    print(len(ground_truth))
    new_edges = ground_truth[~prev_mask]
    prev_edges = ground_truth[prev_mask]
    new_positive = np.sum(new_edges)
    new_negative = len(new_edges)-new_positive
    prev_positive = np.sum(prev_edges)
    prev_negative = len(prev_edges)-prev_positive
    new_positive_ratio = new_positive/len(new_edges)
    prev_positive_ratio = prev_positive/len(prev_edges)
    return new_positive,new_negative,prev_positive,prev_negative,new_positive_ratio,prev_positive_ratio
