...
class Link_Pred_Tasker():
	...
	def build_get_node_feats(self,args,dataset):
		if args.use_2_hot_node_feats:
			...
			def get_node_feats(adj,time):
				return dataset.nodes_feats[time]
	...
	def get_sample(self,idx,test, **kwargs):
		...
		label_adj_edge_ids = tu.get_edges_ids(label_adj['idx'].t().numpy(), self.data.num_nodes)
		indices = np.argsort(label_adj_edge_ids)
		label_adj['idx'] = label_adj['idx'][indices,:]
		label_adj['vals'] = label_adj['vals'][indices]
