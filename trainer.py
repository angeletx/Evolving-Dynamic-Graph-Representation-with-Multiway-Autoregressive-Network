...
class Trainer():
	def __init__(self,args, splitter, gcn, classifier, comp_loss, dataset, num_classes):
		...
		self.mask = np.zeros(self.num_nodes*self.num_nodes, dtype=bool)

		args.gcn_parameters['feats_per_node']=self.tasker.feats_per_node
	...
	def run_epoch(self, split, epoch, set_name, grad):
		...
		for s in split:
			...
			self.mask = tu.update_edges_mask(self.mask, s.hist_adj_list)
			...
			loss = self.comp_loss(predictions,s.label_sp['vals'],parameterList=self.gcn.reg_param_list,l1_weight=self.args.gcn_parameters['l1_weight'],l2_weight=self.args.gcn_parameters['l2_weight'])
			
			if set_name in ['TEST', 'VALID'] and self.args.task == 'link_pred':
				self.logger.log_minibatch(predictions, s.label_sp, loss.detach(), adj = s.label_sp['idx'], prev_edges_mask = self.mask)
			else:
				self.logger.log_minibatch(predictions, s.label_sp, loss.detach(), prev_edges_mask = self.mask)
			if grad:
	...
	def predict(self,hist_adj_list,hist_ndFeats_list,node_indices,mask_list):
		gcn_output = self.gcn(hist_adj_list,
							  hist_ndFeats_list,
							  mask_list)

		if isinstance(gcn_output, tuple):
			nodes_embs = gcn_output
		else:
			nodes_embs = gcn_output
