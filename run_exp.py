...
def build_random_hyper_params(args):
    ...
    if args.model == 'gcn':
        args.num_hist_steps = 0 # sometimes edges are 0 in a single step
...
def build_gcn(args,tasker):
    ...
    elif args.model == 'egcn_man':
        return egcn_man.CEGCN(gcn_args, activation = torch.nn.RReLU(), device = args.device)
    else:
